...
$procedures = ProceduresTable::query()
    ->setSelect([
        ...
        'NAME' => 'ELEMENT.NAME',
        'COLOR' => 'COLOR_ID',
        'COLOR_NAME' => 'COLOR_ELEMENT.NAME'
    ])
    ->fetchAll() ?: [];

foreach ($procedures as &$procedure) {
    if (!empty($procedure['COLOR_ID'])) {
        $procedure['COLOR_NAME'] = AbstractIblockPropertyValuesTable::getElementNameById($procedure['COLOR_ID']);
    }
}
unset($procedure);
...
$procedureColors = array_reduce($procedures, function ($acc, $procedure) {
    $acc[$procedure['ID']][] = [
        'NAME' => $procedure['COLOR_NAME'],
        'COLOR' => $procedure['COLOR_ID']
    ];
    return $acc;
}, []);
...
foreach ($doctors as $doctor) {
    ...
    foreach ($doctor['PROCEDURES'] as $procedureId => $procedureName) {
        ...
        var_dump($colors);
        $colorsHtml = array_reduce($colors, function ($acc, $color) use ($procedureName) {
            $colorCode = $color['NAME'] ?? ''; // Получаем код цвета
            $acc .= "<span class='procedure-name' style='background-color: {$colorCode}; padding: 5px; border-radius: 3px; margin-right: 4px;'>{$procedureName}</span>";
            return $acc;
        }, '');
        $doctorProcedures[] = $colorsHtml;
    }
